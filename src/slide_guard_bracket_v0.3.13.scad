FILE_REVISION = "v0.3.13   |   Lucas Garron   |   2024-09-29";

// $fn = 16; // Enable this for faster iteration in OpenSCAD
$fn = 256;
...
// module multiple_plexiglasses()
// {

//     middle_plexiglass();
//     translate([ 0, -plexiglass_carving_depth_y * 2, 0 ]) middle_plexiglass();
//     translate([ 0, plexiglass_carving_depth_y * 2, 0 ]) middle_plexiglass();
// }
...
middle_support_cylinder_radius = 2;
middle_support_cylinder_height_z = 2;

middle_support_socket_center_y = 29.5;
...
module middle_support()
{
    ...
    translate([ 0, middle_support_socket_center_y, stacking_offset_z + EPSILON / 2 ])
        cylinder(h = middle_support_cylinder_radius + EPSILON, r = middle_support_cylinder_height_z);
}

module middle_support_socket()
{
    translate([ 0, middle_support_socket_center_y + stacking_offset_y, -EPSILON / 2 ]) cylinder(
        h = middle_support_cylinder_radius + EPSILON + 2 * CONTACT_SPACING, r = middle_support_cylinder_height_z);
}
...
module main()
{
    difference()
    {
        ...
        union()
        {
            ...
            translate([ 0, holder_depth_y + EPSILON, -stacking_offset_z - EPSILON ]) clamps(EPSILON);

            translate([ 0, 10, timer_ground_clearance - engraving_depth ]) linear_extrude(engraving_depth * 2)
                text(FILE_REVISION, size = 5, font = "Ubuntu:style=bold", valign = "center", halign = "center");

            middle_support_socket();
        }
    }
}
